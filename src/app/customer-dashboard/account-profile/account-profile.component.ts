...
export class AccountProfileComponent implements OnInit, OnDestroy {
  ...
  public requestDataFromMultipleSources() {
    ...
    const person = { ...this.person, publicName: this.person.name };
  ...
  getProfileSettings() {
    this.newgenSvc
      ...
      .subscribe((x) => {
        this.facebookProfile = x.collection.find(
          (y: any) => this.facebookProfile.typeId == y.typeId
        );
        this.instagramProfile = x.collection.find(
          (y: any) => this.instagramProfile.typeId == y.typeId
        );
        this.tiktokProfile = x.collection.find(
          (y: any) => this.tiktokProfile.typeId == y.typeId
        );
  ...
  getProfilePersonalSettings() {
    this.newgenSvc
      ...
      .subscribe((x) => {
        ...
        this.person.publicPreferredRecognitionName =
          x.collection[0].publicPreferredRecognitionName || '';
  ...
  personProfileUpdate() {
    this.newgenSvc
      .personProfileUpdate(this.person)
      .pipe(takeUntil(this.destroyed$))
      .subscribe((x) => {
        this.loading = false;
      });
